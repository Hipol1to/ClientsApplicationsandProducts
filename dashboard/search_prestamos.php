...
$stmt = $db->prepare("SELECT * FROM prestamos WHERE $searchField LIKE :searchText");
...
$results = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    foreach ($results as $result) {
        echo '<p>' . $result['Id'] . ' - ' . $result['Beneficiario'] . ' - ' . $result['Motivo'] . ' - ' . $result['Status'] . ' - ' . $result['IdCliente'] . '</p>';
    }
} else {
